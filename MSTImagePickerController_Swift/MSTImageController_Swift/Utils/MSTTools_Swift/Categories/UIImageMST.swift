...
internal extension UIImage {
    ...
    /// - Returns: 处理后图片
    func mst_imageByScalingProportionally(maxWidth: CGFloat) -> UIImage? {
        let whratio: CGFloat = size.width / size.height
        
        if whratio > 1 {
            let width = maxWidth
            guard size.width >= width else { return self }
            
            let height: CGFloat = size.height * width / size.width
            let sizeNew: CGSize = CGSize(width: width, height: height)
            
            UIGraphicsBeginImageContext(sizeNew)
            draw(in: CGRect(x: 0, y: 0, width: sizeNew.width, height: sizeNew.height))
            
            let sImg: UIImage? = UIGraphicsGetImageFromCurrentImageContext()
            UIGraphicsEndImageContext()
            
            return sImg
        } else {
            let height: CGFloat = maxWidth
            guard size.height >= height else { return self }
            
            let width: CGFloat = size.width * height / size.height
            let sizeNew: CGSize = CGSize(width: width, height: height)
    ...
    func mst_toNewByColor(_ color: UIColor) -> UIImage {
