...
// MARK: - Macros
internal var kScreenWidth: CGFloat {
    return UIScreen.main.bounds.size.width
}

internal var kScreenHeight: CGFloat {
    return UIScreen.main.bounds.size.height
}

internal var kScreenSize: CGSize {
    return UIScreen.main.bounds.size
}

internal var kScreenScale: CGFloat {
    return UIScreen.main.scale
}

internal var kNavHeight: CGFloat {
    return isIPhoneX ? 88 : 64
}

internal var kTabBarHeight: CGFloat {
    return isIPhoneX ? 83 : 49
}

internal let kSearchBarHeight: CGFloat = 44

internal let kToolBarHeight: CGFloat = 44

internal var isIPhoneX: Bool {
    return kScreenHeight == 812 ? true : false
}

// MARK: - 访问权限
internal class MSTTools {
    enum AuthorType {
        case album
        case capture
        case microphone
    ...
    class func authorization(authorType type: AuthorType) -> AuthorResult {
        ...
        case .capture, .microphone:
            let author: AVAuthorizationStatus = AVCaptureDevice.authorizationStatus(for: type == .capture ? AVMediaType.video : AVMediaType.audio)
            switch author {
            case .denied, .restricted:
                return .denied
            case .authorized:
...
extension MSTTools {
    ...
    // MARK: - 国际化
    class func localizedString(key: String, value: String) -> String {
        guard MSTPhotoConfiguration.shared().isLocalizedString else {
            return value
        }
        
        return Bundle.main.localizedString(forKey: key, value: value, table: "MSTImagePicker")
    }
}
